...
### 从 UI 到 UX 的产品思维（🆕 120字）

老师傅强调："**好 UI ≠ 好 UX**。UI 是界面好不好看，UX 是用起来好不好用。UX 的核心是理解用户场景：用户在什么情况下使用？有什么干扰？有什么情绪？比如 ChefVoiceAI 的界面重构，我们最初追求炫酷的动画效果（UI），但用户反馈在嘈杂的厨房环境中，简洁清晰的按钮更重要（UX）。记住：设计是为了解决问题，不是为了炫技。"
