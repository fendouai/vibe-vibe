...
这段数据，Python 能读懂，JavaScript 能读懂，Java 能读懂，Go 也能读懂。如果不用 JSON，每种语言可能都有自己的"方言"格式，就像古代各地有自己的语言一样，交流起来很困难。

**YAML** 则是一种更人性化的配置格式，它不使用括号，而是通过缩进来表示层级关系，写起来更简洁。很多配置文件（如某些 CI/CD 配置）都使用 YAML 格式。

举个例子：一份点菜单，用 YAML 表示就是这样：
...
YAML 的特点是**像写清单一样自然**——没有花括号、没有逗号，靠缩进表示层级。所以很多人觉得写配置文件用 YAML 比 JSON 更轻松。
...
在学习 HTTP 通信之前，老师傅先给你解释了 **前端** 和 **后端** 这两个概念。
...
你接下来要学的 **HTTP 协议**，就是前端和后端之间"对话"的语言。
...
还有一种常见问题：**数据格式不匹配**。比如后端返回的 JSON 字段是 `userName`，但前端代码里写的是 `username`，或者后端返回的是字符串 `"123"`，前端期望的是数字 `123`。这种情况下，HTTP 状态码可能是 `200 OK`，但数据依然"对不上"，代码还是会报错。AI 知道如何处理格式转换和字段映射，你遇到这类问题告诉它"数据格式对不上"就行。

老师傅特别强调，这套通信机制是整个互联网应用的基础。当你理解了**客户端发送请求、服务器返回响应**这个基本流程后，无论你调用外部 API、实现用户认证、还是处理表单提交，本质上都是在和 HTTP 打交道。这些细节 AI 都会帮你处理。
