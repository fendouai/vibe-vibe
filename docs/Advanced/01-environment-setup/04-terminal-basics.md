...
::: tip 什么是 CLI 软件为什么没有图形界面？

你可能会疑惑：**为什么有些软件没有漂亮的界面，只能在黑乎乎的终端里敲命令？**

这类软件叫 **CLI 软件（Command Line Interface，命令行界面）**。它们没有菜单、没有按钮，一切操作通过输入命令完成。

**为什么开发工具偏爱 CLI？**

1. **更高效**：输入 `pnpm install` 比点开菜单、找到安装按钮快得多
2. **更精确**：命令可以带参数，精确控制行为（如 `pnpm install --save-dev`）
3. **更可脚本化**：可以把命令写成脚本，自动执行一系列操作
4. **更省资源**：不需要加载图形界面，占用内存更少
...
这些命令虽然看起来简陋，但执行速度极快。当你熟练后，会发现效率远超图形界面。
