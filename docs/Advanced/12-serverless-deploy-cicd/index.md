...
- **Node 版本**：平台预装了多个 Node 版本（如 18、20、22、24），推荐使用 24 LTS 版本
...
老师傅顺便告诉你，其实你一直在用的那个开发数据库，现在直接就是生产数据库了——这是小白最省钱的方案，不需要额外租用一个数据库。唯一的注意事项是：开发时产生的测试数据记得清理掉，或者在代码里给数据加个标记（比如 `isTest: true` 字段），上线后过滤掉这些测试数据。
