...
**开发数据库 vs 生产数据库**。老师傅告诉你，在专业团队里，通常会有两套数据库：开发环境用来测试和调试，生产环境给真实用户使用。但对你来说，**一套数据库就够了**——直接在云端数据库上开发，这样部署时不需要迁移数据，省去了很多麻烦。唯一的注意事项是：开发时千万别写 `DELETE` 或 `DROP` 这种危险操作，否则会把真实数据也删掉。
